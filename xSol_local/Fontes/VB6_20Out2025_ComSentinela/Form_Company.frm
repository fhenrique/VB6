...
Begin VB.Form Form_Company 
   ...
   End
End
Attribute VB_Name = "Form_Company"
...
Private Sub Form_Activate()
    Text_Company.Font.Bold = MDIForm_Xsol.acessibilidade_TextoNegrito
...
Private Function Return_SqlString(queryType As Integer, Optional param1 As Integer, Optional param2 As String) As String
    ...
    If queryType = 1 Then
        Return_SqlString = "SELECT * FROM Empresa ORDER BY Tipo_Cliente"
    ElseIf queryType = 2 Then
        Return_SqlString = "SELECT * FROM Empresa WHERE Codigo = " & param1
    ElseIf queryType = 3 Then
        Return_SqlString = "SELECT COUNT(*) AS qtt FROM Clientes WHERE Solicitante = '" & param2 & "'"
    End If
End Function
...
Private Sub Fill_Grid(rs As ADODB.Recordset, Optional code As Integer)
    ...
    With Grid_Main
        ...
        Do Until rs.EOF
            .AddItem (rs.Fields("codigo") & vbTab & rs.Fields("Tipo_Cliente"))
...
Sub Show_Record_Detail(rs As ADODB.Recordset)
    ...
    Text_Company.Text = rs.Fields("Tipo_Cliente")
...
Private Sub Cmd_Edita_Click()
   If Have_Reference() Then
      MsgBox "Não é possível editar " & Text_Company & " pois há cliente(s) que referencia(m) esta empresa", vbInformation, "Informação"
      Exit Sub
   End If
...
Private Sub Record_Record(rs As ADODB.Recordset)
   rs.Fields("Tipo_Cliente").value = Text_Company
End Sub

Private Sub Cmd_Exclui_Click()
   If Have_Reference() Then
      MsgBox "Não é possível remover " & Text_Company & " pois há cliente(s) que referencia(m) esta empresa", vbInformation, "Informação"
      Exit Sub
   End If
   If MsgBox("Confirma a Exclusão de " & Text_Company & " ? ", vbYesNo, "Excluir Registros") = vbYes Then
...
Private Function Have_Reference() As Boolean
    ...
    Set rs = Return_Recordset(Return_SqlString(3, param2:=Text_Company))
...
Private Function Mandatory_Fields_Filled() As Boolean
    ...
    If Trim(Text_Company) = "" Then
       MsgBox "É necessário digitar a Empresa! ", vbOKOnly, "XSol Aquecedores"
       Text_Company.SetFocus
...
Private Sub Object_Controller(cType As Integer)
    ...
    Text_Company.enabled = cType >= 3
    
    Text_Company.BackColor = IIf(cType >= 3, vbYellow, vbWhite)
