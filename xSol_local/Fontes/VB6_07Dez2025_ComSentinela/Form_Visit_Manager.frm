...
'        Mask_Dt_Prox_Visita.BackColor = COLOR_BTNFACE
    ...
    If Insert Then
        Call Insert_New_Visit(Txt_Cod_Cliente.Text, Grid_Prox_Visita.TextMatrix(Grid_Prox_Visita.RowSel, 8))
    Else
    ...
    MsgBox "Visita gravada!", vbOKOnly, "XSOL Aquecedores"
...
Private Function consultaEnderecoCliente(strCliente As String, Optional strCodEndereco As String) As Recordset
    Set consultaEnderecoCliente = Return_Recordset(Return_SqlString(11, strCliente, strCodEndereco))
End Function
...
Private Sub Insert_New_Visit(clientCode As String, Optional strCodEndereco As String)
    Dim rs As ADODB.Recordset
    Dim rs_enderecos As Recordset
    Dim strEnderecoAtendimento As String
    Dim intProximoCodVisita As Long
    
    If strCodEndereco <> "" Then
        Set rs_enderecos = consultaEnderecoCliente(clientCode, strCodEndereco)
    Else
    ...
    If strCodEndereco <> "" Then
        Set rs = Return_Recordset(Return_SqlString(2, clientCode, strCodEndereco))
    Else
    ...
    'talvez aqui
    If strCodEndereco <> "" Then
        Call Expire_Visit(clientCode, strCodEndereco)
    Else
    ...
    'rs.Fields("Cod_Endereco") = Grid_Prox_Visita.TextMatrix(Grid_Prox_Visita.RowSel, 3)
...
Private Sub But_Search_Click()
    ...
    Dim clienteCPF As String
    Dim clienteNome As String
    
    If (Txt_Search_Code.Text = "") And (Txt_Search_CPF.Text = "") And (Txt_Search_Name.Text = "") Then
        MsgBox "Informe um filtro para a pesquisa!", vbOKOnly, "Sistema XSOL"
        Exit Sub
    End If
    
    
    clienteCPF = Txt_Search_CPF.Text
    
    If (clienteCPF <> "") And (Mid(clienteCPF, 4, 1) <> ".") Then
        clienteCPF = ""
        clienteCPF = Mid(Txt_Search_CPF.Text, 1, 3) & "." & Mid(Txt_Search_CPF.Text, 4, 3) & "." & Mid(Txt_Search_CPF.Text, 7, 3) & "-" & Mid(Txt_Search_CPF.Text, 10, 2)
    End If
    
    With Grid_Prox_Visita
        Call Fill_Grid(Return_Recordset(Return_SqlString(4, Txt_Search_Code.Text, clienteCPF, Txt_Search_Name.Text)))
    End With
End Sub
...
Private Sub Grid_Hist_DblClick()
    ...
    Else
            'faz a consulta com o codigo do endereco do gri_prox_visita (ult. coluna)
            Set rsEndereco = Return_Recordset(Return_SqlString(12, Grid_Prox_Visita.TextMatrix(Grid_Prox_Visita.RowSel, 8)))
            
            'coloca o resultado da consulta na variavel
            enderecoAtendimento = rsEndereco.Fields("endereco")
            rsEndereco.Close
...
Private Sub Txt_Search_Code_GotFocus()
    Txt_Search_CPF.Text = ""
    Txt_Search_Name.Text = ""
End Sub
...
Private Sub Txt_Search_CPF_GotFocus()
    Txt_Search_Code.Text = ""
    Txt_Search_Name.Text = ""
End Sub
...
Private Sub Txt_Search_Name_GotFocus()
    Txt_Search_CPF.Text = ""
    Txt_Search_Code.Text = ""
End Sub
...
Private Sub Fill_Grid(rs As ADODB.Recordset, Optional code As String)
    ...
    With Grid_Prox_Visita
        .Rows = 2
        Do Until rs.EOF
            'parou aqui, colocar antes o cod
            '.AddItem (Get_Field_Val(rs.Fields("v.codigo")) & vbTab & rs.Fields("c.Codigo") & vbTab & Get_Field_Val(rs.Fields("nome_cli")) & vbTab + CStr(Get_Field_Val(rs.Fields("Endereco.codigo"))) & vbTab + Get_Field_Val(rs.Fields("dt_prox_visita")) & vbTab & Get_Field_Val(rs.Fields("hora_prox_visita")) & vbTab & Get_Field_Val(rs.Fields("fone_celular")) & vbTab & Get_Field_Val(rs.Fields("foneres")) & vbTab & Get_Field_Val(rs.Fields("endereco_da_visita")))
            .AddItem (Get_Field_Val(rs.Fields("v.codigo")) & vbTab & rs.Fields("c.Codigo") & vbTab & Get_Field_Val(rs.Fields("nome_cli")) & vbTab + Get_Field_Val(rs.Fields("dt_prox_visita")) & vbTab & Get_Field_Val(rs.Fields("hora_prox_visita")) & vbTab & Get_Field_Val(rs.Fields("fone_celular")) & vbTab & Get_Field_Val(rs.Fields("foneres")) & vbTab & Get_Field_Val(rs.Fields("endereco_da_visita")) & vbTab & Get_Field_Val(rs.Fields("Endereco.codigo")))
            ...
            End If
            If code <> "" Then
                If code = rs.Fields("v.codigo") Then
                    selectedRow = count
                End If
                count = count + 1
            End If
...
Private Sub Configure_Grid()
    With Grid_Prox_Visita
        ...
        .ColWidth(5) = 1500
        .ColWidth(6) = 1500
        .ColWidth(7) = 7500 'Endereco do atendimento
        .ColWidth(8) = 1500 'Endereco do atendimento
...
Private Function Return_SqlString(queryType As Integer, Optional param1 As String, Optional param2 As String, Optional param3 As String) As String
    ...
    If queryType = 1 Then
        ...
        Return_SqlString = "SELECT v.codigo, c.codigo, nome_cli, endereco.codigo,dt_prox_visita,hora_prox_visita, fone_celular, foneres, endereco_da_visita FROM (Clientes AS c LEFT JOIN (SELECT * FROM Visitas WHERE dt_Expiracao IS NULL)  AS v ON c.codigo = v.cod_cliente) INNER JOIN Endereco ON c.Codigo = Endereco.Cod_Cliente WHERE (((v.Dt_Expiracao) Is Null) AND ((c.Aquivo_Morto)='N')) ORDER BY v.Dt_Prox_Visita, v.Hora_Prox_Visita, c.Nome_Cli"
        'Return_SqlString = "SELECT c.codigo, nome_cli, dt_prox_visita,hora_prox_visita, fone_celular, foneres, endereco_da_visita FROM (Clientes AS c LEFT JOIN (SELECT * FROM Visitas WHERE dt_Expiracao IS NULL)  AS v ON c.codigo = v.cod_cliente) INNER JOIN Endereco ON c.Codigo = Endereco.Cod_Cliente WHERE (((v.Dt_Expiracao) Is Null) AND ((c.Aquivo_Morto)='N')) group by c.codigo, nome_cli, dt_prox_visita,hora_prox_visita, fone_celular, foneres, endereco_da_visita ORDER BY v.Dt_Prox_Visita, v.Hora_Prox_Visita, c.Nome_Cli"
        
    ElseIf queryType = 2 Then 'Tabela Visitas filtrando Cod_Cliente, ordenano por código
        If param2 <> "" Then
            Return_SqlString = "SELECT * FROM Visitas WHERE Cod_Cliente = " & param1 & " and Cod_Endereco = " & param2
        Else
    ...
    ElseIf queryType = 4 Then 'Tabela Clientes com Join em Visitas
        ...
        If param1 <> "" Then
            Return_SqlString = "SELECT top 30 v.codigo, c.codigo, nome_cli, endereco.codigo,dt_prox_visita,hora_prox_visita, fone_celular, foneres, endereco_da_visita FROM (Clientes AS c LEFT JOIN (SELECT * FROM Visitas WHERE dt_Expiracao IS NULL)  AS v ON c.codigo = v.cod_cliente) INNER JOIN Endereco ON (c.Codigo = Endereco.Cod_Cliente) WHERE (((v.Dt_Expiracao) Is Null) AND ((c.Aquivo_Morto)='N')) and c.codigo = " & param1 & " ORDER BY v.Dt_Prox_Visita, v.Hora_Prox_Visita, c.Nome_Cli"
        End If
        If param2 <> "" Then
            Return_SqlString = "SELECT top 30 v.codigo, c.codigo, nome_cli, endereco.codigo,dt_prox_visita,hora_prox_visita, fone_celular, foneres, endereco_da_visita FROM (Clientes AS c LEFT JOIN (SELECT * FROM Visitas WHERE dt_Expiracao IS NULL)  AS v ON c.codigo = v.cod_cliente) INNER JOIN Endereco ON (c.Codigo = Endereco.Cod_Cliente) WHERE (((v.Dt_Expiracao) Is Null) AND ((c.Aquivo_Morto)='N')) and c.cpf = '" & param2 & "' ORDER BY v.Dt_Prox_Visita, v.Hora_Prox_Visita, c.Nome_Cli"
        End If
        
        If param3 <> "" Then
            Return_SqlString = "SELECT top 30 v.codigo, c.codigo, nome_cli, endereco.codigo,dt_prox_visita,hora_prox_visita, fone_celular, foneres, endereco_da_visita FROM (Clientes AS c LEFT JOIN (SELECT * FROM Visitas WHERE dt_Expiracao IS NULL)  AS v ON c.codigo = v.cod_cliente) INNER JOIN Endereco ON (c.Codigo = Endereco.Cod_Cliente) WHERE (((v.Dt_Expiracao) Is Null) AND ((c.Aquivo_Morto)='N')) and ucase(nome_cli) like  '%" & UCase(param3) & "%' ORDER BY v.Dt_Prox_Visita, v.Hora_Prox_Visita, c.Nome_Cli"
        End If
    ...
    ElseIf queryType = 11 Then
        If param2 <> "" Then
            Return_SqlString = "SELECT endereco & '-' & bairro & '-' & cidade as endereco From endereco Where Cod_cliente = " & param1 & " and Codigo = " & param2
        Else
            Return_SqlString = "SELECT endereco & '-' & bairro & '-' & cidade as endereco From endereco Where Cod_cliente = " & param1
        End If
    ElseIf queryType = 12 Then
        Return_SqlString = "Select endereco & ', ' & bairro & ', ' & cidade as endereco from Endereco Where codigo = " & param1
    End If
End Function
...
Private Sub Show_Visit_Hist()
    With Grid_Prox_Visita
        Txt_Cod_Cliente = Grid_Prox_Visita.TextMatrix(.RowSel, 1)
