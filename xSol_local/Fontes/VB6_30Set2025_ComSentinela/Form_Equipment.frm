...
Private Sub Form_Load()
   ...
   Call Configure_Grid
   'Call Show_Customer_Equipment(Return_Recordset(Return_SqlString(1, Str(Cod_Cli))), Str(IIf(Cod_Equipment = "", "-1", Cod_Equipment)))
   
   
   'chamar aqui uma nova funcao que mostre a porra dos equipamentos do cliente no grid
...
Private Sub Cmd_Salva_Click()
    ...
    Dim rstelhado As ADODB.Recordset
    ...
    If Cmd_Inclui.Caption = "&Cancela" Then
        ...
        rs.Fields("Cod_Endereco") = txtCodEnderecos.Text
        ...
        'ajuste do telhado
        Set rstelhado = Return_Recordset(Return_SqlString(6, Text_Cod_Cliente.Text))
        rstelhado.AddNew
        rs.Fields("tipo_telhado") = txtTipoTelhado.Text
        
        rstelhado.Update
        rstelhado.Close
        
    Else
        Set rs = Return_Recordset(Return_SqlString(7, Text_Cod.Text))
    End If
...
Private Function Return_SqlString(queryType As Integer, Optional param1 As String, Optional param2 As String, Optional param3 As String) As String
    ...
    If queryType = 1 Then
        'Return_SqlString = "SELECT distinct(a.codigo), a.cod_cliente, a.equipamento, a.cod_endereco, a.exibenaficha, b.tipo_telhado FROM Equipamento a Inner JOIN Endereco b on b.cod_cliente = a.cod_cliente Where a.cod_cliente = " & param1 & " ORDER BY a.Codigo"
