...
Begin VB.Form Form_Customer 
   ...
   Begin VB.Frame Frame1 
      ...
      Top             =   45
      Width           =   8055
      Begin VB.Frame panInfoAdicional 
         BorderStyle     =   0  'None
         Height          =   3630
         Left            =   225
         TabIndex        =   71
         ...
         Visible         =   0   'False
         Width           =   7605
         Begin VB.TextBox txtInfoAdicional 
            Height          =   3525
            Left            =   30
            MaxLength       =   3000
            MultiLine       =   -1  'True
         ...
         End
      End
      Begin VB.CommandButton btnInfoAdicional 
...
Private Sub btnInfoAdicional_Click()
    If panInfoAdicional.Visible = True Then
        panInfoAdicional.Visible = False
    Else
        panInfoAdicional.Visible = True
    End If
...
Private Sub Cmd_Serv_Click()
    ...
    global_Ficha_telefone1 = strFoneRes
    global_Ficha_telefone2 = strFoneCom
    global_Ficha_telefone2 = strFoneCel
    
    
    
    global_abrindoForm = True
...
Private Sub Configure_Grid()
    ...
    With Grid_Customer
        ...
        .TextMatrix(0, 5) = "Arq. Morto"
        .TextMatrix(0, 6) = "Justificativa"
        .RowHeight(1) = 0
    End With
...
Private Function Return_SqlString(queryType As Integer, Optional param1 As String, Optional param2 As String, Optional param3 As String, Optional param4 As String) As String
    ...
    If queryType = 1 Then
        Return_SqlString = "SELECT top 50 codigo,nome_cli,cpf, cnpj,fone_celular, Aquivo_Morto, Aquivo_Morto_Justificativa FROM Clientes AS c"
    ElseIf queryType = 2 Then
    
        Return_SqlString = "SELECT * FROM Clientes AS c WHERE codigo = " & param1
        
    ElseIf queryType = 3 Then
        Return_SqlString = "SELECT top 50 c.codigo,nome_cli,cpf, cnpj, fone_celular,Aquivo_Morto, Aquivo_Morto_Justificativa FROM Clientes c LEFT JOIN Endereco e ON c.codigo = e.cod_cliente"
        If param1 <> "" Then
    ...
    ElseIf queryType = 4 Then
        'Return_SqlString = "SELECT top 100 * FROM Clientes AS c"
        Return_SqlString = "SELECT top 50 * FROM Clientes AS c"
    ...
    With Grid_Customer
        ...
        Do Until rs.EOF
            If lastCode <> rs.Fields("codigo") Then
               ...
               count = count + 1
               .AddItem (rs.Fields("codigo") & vbTab & IIf(IsNull(rs.Fields("Nome_Cli")), "", rs.Fields("Nome_Cli")) & vbTab & IIf(IsNull(rs.Fields("cpf")), "", rs.Fields("cpf")) & vbTab & IIf(IsNull(rs.Fields("cnpj")), "", rs.Fields("cnpj")) & vbTab & rs.Fields("fone_celular") & vbTab & rs.Fields("Aquivo_Morto") & vbTab & rs.Fields("Aquivo_Morto_Justificativa"))
               If code <> "" Then
...
'    Text_Tipo_Equip.Text = UCase(IIf(IsNull(rs.Fields("Tipo_Eq_Mod")), "", rs.Fields("Tipo_Eq_Mod")))
    ...
    txtInfoAdicional.Text = IIf(IsNull(rs.Fields("INFO_ADICIONAL")), "", rs.Fields("INFO_ADICIONAL"))
...
Private Sub Record_Record(rs As ADODB.Recordset)
   ...
   If Trim(txtInfoAdicional.Text) = "" Then
      rs.Fields("INFO_ADICIONAL").value = Null
   Else
      rs.Fields("INFO_ADICIONAL").value = txtInfoAdicional.Text
   End If
...
Private Sub Grid_Address_Click()
    ...
    strCodEndereco = Grid_Address.TextMatrix(Grid_Address.RowSel, 0)
...
Private Sub Text_Cnpj_GotFocus()
    Text_Cpf.Text = ""
End Sub
