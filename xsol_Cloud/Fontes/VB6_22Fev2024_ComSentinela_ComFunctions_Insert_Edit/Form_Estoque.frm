...
Private Sub Configure_Grid()
    With gridVisaoEstoqueProduto
        ...
        .ColWidth(0) = 650
        .ColWidth(1) = 3200
        .ColWidth(2) = 980 'Dt. Entrada
        .ColWidth(3) = 900
        .ColWidth(4) = 1000
        .ColWidth(5) = 700 '% Lucro
        .ColWidth(6) = 980
        .ColWidth(7) = 800 'Val. Saída
        .ColWidth(8) = 870
        .ColWidth(9) = 800 'Liberado
        .ColWidth(10) = 2370 'Tecnico
        .ColWidth(11) = 600 'Saldo
        ...
        .TextMatrix(0, 2) = "Dt. Entrada"
        .TextMatrix(0, 3) = "Vl. Entrada"
        .TextMatrix(0, 4) = "Qtd. Entrada"
        .TextMatrix(0, 5) = "% Lucro"
        .TextMatrix(0, 6) = "Dt. Saída"
        .TextMatrix(0, 7) = "Vl. Saída"
        .TextMatrix(0, 8) = "Qtd. Saída"
        .TextMatrix(0, 9) = "Liberado"
        .TextMatrix(0, 10) = "Técnico"
        .TextMatrix(0, 11) = "Saldo"
...
Private Sub Fill_Grid_VisaoEstoque(rs As ADODB.Recordset, Optional code As String)
    ...
    With gridVisaoEstoqueProduto
        ...
        Do Until rs.EOF
            ...
            .AddItem (rs.Fields("codigo") & vbTab & IIf(IsNull(rs.Fields("produto")), "", rs.Fields("produto")) & vbTab & IIf(IsNull(rs.Fields("Dt_Ent")), "", rs.Fields("Dt_Ent")) & vbTab & IIf(IsNull(rs.Fields("Valor_Ent")), "", Format(Str(rs.Fields("Valor_Ent")), "###,###,##0.00")) & vbTab & IIf(IsNull(rs.Fields("Qtd_Ent")), "", rs.Fields("Qtd_Ent")) & vbTab & IIf(IsNull(rs.Fields("Porcent_Lucro")), "", rs.Fields("Porcent_Lucro")) & vbTab & IIf(IsNull(rs.Fields("Dt_Saida")), "", rs.Fields("Dt_Saida")) & vbTab & IIf(IsNull(rs.Fields("Valor_Saida")), Format("0", "###,###,##0.00"), Format(rs.Fields("Valor_Saida"), "###,###,##0.00")) & vbTab & IIf(IsNull(rs.Fields("Qtd_Saida")), "", rs.Fields("Qtd_Saida")) & vbTab & IIf(IsNull(rs.Fields("Liberado")), "", rs.Fields("Liberado")) & vbTab & IIf(IsNull(rs.Fields("Nome_Tecnico")), "", rs.Fields("Nome_Tecnico")) & vbTab & IIf(IsNull(rs.Fields("Saldo")), "", rs.Fields("Saldo")))
            
            If rs.Fields("Dt_Ent") <> "" Then
...
Public Function Return_SqlString(queryType As Integer, Optional param1 As String, Optional param2 As String) As String
    ...
    ElseIf queryType = 9 Then
        If param1 = "..." Then
           param1 = "0"
        End If
        Return_SqlString = "SELECT * FROM Estoque WHERE CodMaterial = " & param1
...
Private Sub Cmd_Exclui_Click()
   If gridVisaoEstoqueProduto.Rows > 2 Then
    If MsgBox("Confirma a Exclusão de estoque do produto " & txtProdutoDescricao.Text & " ? ", vbYesNo, "Excluir Registros") = vbYes Then
       Call Return_Recordset(Return_SqlString(2, gridVisaoEstoqueProduto.TextMatrix(gridVisaoEstoqueProduto.RowSel, 0))).Delete
...
Private Sub Cmd_Salva_Click()
    ...
    If movimentoEntrada Then
        ...
        If Trim(txtEntradaValor.Text) = "" Then
          MsgBox "O valor do produto não foi informado.", vbOKOnly, "Xsol Aquecedores"
          'txtEntradaValor.SetFocus
          Exit Sub
        End If
