...
Private Sub Form_Load()
   ...
   Call Show_Customer_Equipment(Return_Recordset(Return_SqlString(8, Str(Cod_Cli))))
...
Private Sub Cmd_Print_Click()
    ...
    If Option_Assistencia Then
        ...
        If Chk_Phone.value = 1 Then
            relFicha_Assistencia_Tecnica.lblTelRes.Caption = Form_Customer.Grid_Address.TextMatrix(2, 6)
            relFicha_Assistencia_Tecnica.lblTelCom.Caption = Form_Customer.Grid_Address.TextMatrix(2, 7)
            relFicha_Assistencia_Tecnica.lblCelular.Caption = Form_Customer.Grid_Address.TextMatrix(2, 8)
        Else
        ...
        If Form_Service.Grid_Equipment.Rows > 2 Then
            ...
            'Se só tem 1 equipamento
            If Grid_Equipment.Rows - 2 = 1 Then
                
                'testar isso aqui com ficha que só tem 1 equipamento
        ...
        If (Combo_Servico2.Text = "") And (Form_Service.Grid_Equipment.Rows - 2 = 1) Then
            ...
            If (Grid_Equipment.TextMatrix(2, 2) = "N") Then
                ...
                relFicha_Assistencia_Tecnica.lblViewEquipamento1.Top = 2450
                relFicha_Assistencia_Tecnica.lblEquipamento1.Top = 2450
        ...
        '2 servicos
        '2 equipamentos
        If (Form_Service.Grid_Equipment.Rows - 2 = 2) And (Combo_Servico2.Text <> "") Then
...
Private Function Return_SqlString(queryType As Integer, Optional param1 As String, Optional param2 As String, Optional param3 As String) As String
    ...
    ElseIf queryType = 16 Then
        'Return_SqlString = "SELECT DiasNaoDisponivel, MesNaoDisponivel FROM Tecnico WHERE Codigo = " & param1 & " AND (DiasNaoDisponivel LIKE '*" & Mid(Mask_Dt_Serv.Text, 1, 2) & "*' OR DiasNaoDisponivel = 'M')"
        'Return_SqlString = "SELECT count(*) as qtd FROM Tecnico WHERE Codigo = " & param1 & " AND (DiasNaoDisponivel LIKE '*" & Mid(Mask_Dt_Serv.Text, 1, 2) & "*' OR DiasNaoDisponivel = 'M')"
...
Private Sub Show_Customer_Address(rs As ADODB.Recordset, Optional code As Integer)
    ...
    With Grid_Address
        ...
        Do Until rs.EOF
            .AddItem (rs.Fields("codigo") & vbTab & IIf(IsNull(rs.Fields("Endereco")), "", rs.Fields("Endereco")) & vbTab & IIf(IsNull(rs.Fields("Cidade")), "", rs.Fields("Cidade")))
            If code = rs.Fields("codigo") Then
                selectedRow = .Rows - 1
            End If
            rs.MoveNext
