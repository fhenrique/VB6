...
Private Sub Cmd_Exclui_Click()
    If MsgBox("Confirma a Exclusão do Endereço " & Text_End.Text & " do Cliente " & Text_Nome_Cliente.Text & " ? ", vbYesNo, "Excluir Registros") = vbYes Then
        ...
        If Not cloud_delete("Endereco", "Codigo = '" & Text_Cod.Text & "'") Then
           MsgBox "Erro ao tentar excluir", vbCritical, "XSOL Aquecedores"
        End If
...
On Error GoTo erro
  ...
  cloud_sql = "SELECT * FROM `Atendimento` WHERE Cod_End = " & Text_Cod.Text
...
erro:
    If cloud_status_conexao = 0 Then
        ...
        If ConectaMySQL Then
            GoTo inicio
        Else
            MsgBox "Desconectado do banco de dados", vbCritical, "XSOL Aquecedores"
            End
        End If
    Else
        GoTo inicio
