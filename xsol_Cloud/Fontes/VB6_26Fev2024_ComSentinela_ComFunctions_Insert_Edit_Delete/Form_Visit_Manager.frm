...
Private Sub But_Salvar_Click()
    ...
    cloud_sql = "SELECT count(*) as qtd FROM `Clientes` c LEFT JOIN (SELECT * FROM `Visitas` WHERE dt_Expiracao IS NULL) v ON c.codigo = v.cod_cliente WHERE v.Dt_Expiracao IS NULL AND c.Aquivo_Morto = 'N' AND v.Dt_Prox_Visita = '" & Format(Mask_Dt_Prox_Visita, "yyyy-MM-dd") & "'"
    Set rsQtd = cloud_conexao.Execute(cloud_sql)
...
Private Function consultaEnderecoCliente(strCliente As String) As Recordset
    '"SELECT `Endereco` & '-' & bairro & '-' & cidade as endereco From `Endereco` Where Cod_cliente = " & param1
    cloud_sql = "Select Endereco from `Endereco` Where Cod_Cliente = " & strCliente
    Set consultaEnderecoCliente = cloud_conexao.Execute(cloud_sql)
End Function
...
On Error GoTo erro
    ...
    'Não dá pra usar o cloud_insert porque a vírgula tem no endereço
    'Call cloud_Insert("Visita", "Cod_Cliente,Dt_Criacao,Dt_Prox_Visita,Hora_Prox_Visita,Obs,Endereco_Da_Visita,Cod_Visita", clientCode & textoComVirgulaEsquerda(Format(Now, "Short Date")) & textoComVirgulaEsquerda(Mask_Dt_Prox_Visita.value) & textoComVirgulaEsquerda(Mask_Hora_Prox_Visita.Text) & textoComVirgulaEsquerda(Txt_Obs.Text) & textoComVirgulaEsquerda(strEnderecoAtendimento) & textoComVirgulaEsquerda(Str(intProximoCodVisita)))
    
    cloud_sql = "Insert into `Visitas` (Cod_Cliente,Dt_Criacao,Dt_Prox_Visita,Hora_Prox_Visita,Obs,Endereco_Da_Visita,Cod_Visita) Values(" & clientCode & ",'" & Format(Now, "Short Date") & "','" & Mask_Dt_Prox_Visita.value & "','" & Mask_Hora_Prox_Visita.Text & "','" & Txt_Obs.Text & "','" & strEnderecoAtendimento & "'," & Str(intProximoCodVisita) & ")"
...
erro:
    ...
    If ConectaMySQL Then
        GoTo retenta
    Else
        MsgBox "Desconectado do banco na nuvem, reinicie o sistema!", vbCritical, "XSOL Aquecedores"
        End
    End If
...
On Error GoTo erro
    If clientCode <> "" Then
        ...
        cloud_sql = "Update `Visitas` Set Dt_Expiracao =" & Format(DateTime.Now, "yyyy-MM-dd ") & " Where Dt_Expiracao is null and cod_cliente = " & clientCode
retenta:
        cloud_conexao.Execute (cloud_sql)
        Exit Sub
    Else
        cloud_sql = "Update `Visitas` Set Dt_Expiracao =" & Format(DateTime.Now, "yyyy-MM-dd ") & " Where cod_cliente = " & clientCode
...
erro:
    If ConectaMySQL Then
        GoTo retenta:
    Else
        MsgBox "Erro ao tentar reconectar ao banco na nuvem", vbCritical, "XSOL Aquecedores"
        End
    End If
End Function
...
retenta:
    ...
    Set cloud_registro = cloud_conexao.Execute(strSql)
    Set Return_Recordset3 = cloud_registro
    Exit Function
erro:
    If ConectaMySQL Then
        GoTo retenta
    Else
        MsgBox "Erro na conexão ao banco na nuvem", vbCritical, "XSOL Aquecedores"
        End
    End If
End Function
...
Private Sub Form_Load()
    ...
    Call carregaQtdAtendimentosNoDia 'chega até aqui
...
Private Sub Fill_Grid(rs As ADODB.Recordset, Optional code As String)
    ...
    Dim c_Codigo As String
    Dim Nome_Clie As String
    Dim Dt_Prox_Visita As String
    Dim Hora_Prox_Visita As String
    Dim Fone_Celular As String
    Dim Fone_Res As String
    Dim Endereco_Da_Visita As String
    
    
    'If IsNull(rs.Fields("Hora_Prox_Visita")) Then
    '    Hora_Prox_Visita = ""
    'Else
    '    Hora_Prox_Visita = rs.Fields("Hora_Prox_Visita")
    'End If
    ...
    With Grid_Prox_Visita
        ...
        Do Until rs.EOF
            ...
            v_Codigo = IIf(IsNull(rs.Fields("v_Codigo")), "", rs.Fields("v_Codigo"))
            c_Codigo = IIf(IsNull(rs.Fields("c_Codigo")), "", rs.Fields("c_Codigo"))
            Nome_Cliente = IIf(IsNull(rs.Fields("Nome_Cli")), "", rs.Fields("Nome_Cli"))
            Dt_Prox_Visita = IIf(IsNull(rs.Fields("Dt_Prox_Visita")), "", rs.Fields("Dt_Prox_Visita"))
            Hora_Prox_Visita = IIf(IsNull(rs.Fields("Hora_Prox_Visita")), "", rs.Fields("Hora_Prox_Visita"))
            Fone_Celular = IIf(IsNull(rs.Fields("Fone_Celular")), "", rs.Fields("Fone_Celular"))
            Fone_Res = IIf(IsNull(rs.Fields("Fone_Res")), "", rs.Fields("Fone_Res"))
            Endereco_Da_Visita = IIf(IsNull(rs.Fields("Fone_Res")), "", rs.Fields("Fone_Res"))
        
        
            'Format(MyTime, "h:m:s")
            .AddItem (v_Codigo & vbTab & c_Codigo & vbTab & Nome_Cliente & vbTab + Dt_Prox_Visita & vbTab + Hora_Prox_Visita & vbTab & Fone_Celular & vbTab & Fone_Res & vbTab & Endereco_Da_Visita)
            ...
            End If
            If code <> "" Then
                If code = rs.Fields("c_Codigo") Then
                    selectedRow = count
                End If
                count = count + 1
            End If
...
Private Function Return_SqlString(queryType As Integer, Optional param1 As String, Optional param2 As String, Optional param3 As String) As String
    ...
    If queryType = 1 Then
        Return_SqlString = "Select v.Codigo as v_Codigo, c.Codigo as c_Codigo, c.Nome_Cli, v.Dt_Prox_Visita, v.Hora_Prox_Visita, c.Fone_Celular, c.Fone_Res, v.Endereco_Da_Visita From Clientes c Left Join Visitas v ON v.cod_cliente = c.codigo Where c.Aquivo_Morto = 'N' and v.dt_Expiracao is NULL Order by  v.Dt_Expiracao, v.Hora_Prox_Visita, c.Nome_Cli limit 50"
    ...
    ElseIf queryType = 4 Then 'Tabela Clientes com Join em Visitas
        ...
        Return_SqlString = "Select v.Codigo as v_Codigo, c.Codigo as c_Codigo, c.Nome_Cli, v.Dt_Prox_Visita, v.Hora_Prox_Visita, c.Fone_Celular, c.Fone_Res, v.Endereco_Da_Visita From Clientes c Left Join Visitas v ON v.cod_cliente = c.codigo where v.Dt_Expiracao IS NULL"
        
        If param1 <> "" Then
            whereClause = IIf(whereClause = "", " AND ", " WHERE ") & " c.codigo = " & param1 & " AND c.Aquivo_Morto = 'N'"
        End If
        If param2 <> "" Then
            whereClause = IIf(whereClause = "", " AND ", " WHERE ") & " cpf LIKE '%" & param2 & "%'"
        End If
        If param3 <> "" Then
            whereClause = IIf(whereClause = "", " AND ", " WHERE ") & " nome_cli LIKE '%" & param3 & "%'"
        End If
        

        Return_SqlString = Return_SqlString & whereClause & " ORDER BY Dt_Prox_Visita, Hora_Prox_Visita limit 50"
    ...
    ElseIf queryType = 6 Then 'Tabela Visitas, filtrando Dt_Prox_Visita
        Return_SqlString = "SELECT count(*) as qtd FROM `Clientes` c LEFT JOIN (SELECT * FROM `Visitas` WHERE dt_Expiracao IS NULL) v ON c.codigo = v.cod_cliente WHERE v.Dt_Expiracao IS NULL AND c.Aquivo_Morto = 'N' AND v.Dt_Prox_Visita = '" & Format(param1, "yyyy-MM-dd") & "'"
...
erro:
    If ConectaMySQL Then
        GoTo retenta
    Else
        MsgBox "Erro ao tentar reconectar com o Cloud, reinicie o sistema!", vbCritical, "XSOL Aquecedores"
        End
    End If
End Function
...
Private Sub Show_Visit_Detail(rs As ADODB.Recordset)
    ...
    If IsNull(rs.Fields("Dt_Prox_Visita")) Then
        Mask_Dt_Prox_Visita.value = -1
    Else
        Mask_Dt_Prox_Visita.value = rs.Fields("Dt_Prox_Visita")
    End If
    
    If IsNull(rs.Fields("Dt_Criacao")) Then
        Txt_Dt_Criacao.value = -1
    Else
