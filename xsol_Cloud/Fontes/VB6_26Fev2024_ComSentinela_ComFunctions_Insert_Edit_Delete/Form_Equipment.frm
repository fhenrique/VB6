...
Private Sub Cmd_Exclui_Click()
    If MsgBox("Confirma a Exclusão do Equipamento " & Text_Equip.Text & " do Cliente " & Text_Nome_Cliente.Text & " ? ", vbYesNo, "Excluir Registros") = vbYes Then
        ...
        'SELECT * FROM `Atendimento` WHERE Cod_Equip = " & param1
        
        cloud_sql = "Select * From `Atendimento` Where Cod_cliente = " & Text_Cod_Cliente.Text & " And Cod_Equip = " & Text_Cod.Text
        
        If cloud_TemReferencia(cloud_sql) Then
            MsgBox "Não é possível sua remoção pois há uma OS que referencia a este endereço", vbInformation, "Informação"
            Exit Sub
        Else
            If Not cloud_delete("Equipamento", "Codigo = '" & Text_Cod.Text & "' And Cod_Cliente = '" & Text_Cod_Cliente.Text & "'") Then
                MsgBox "Não foi possível realizar a exclusão! Comunique ao desenvolvedor", vbCritical, "XSOL Aquecedores"
            End If
        End If
        
        'Call Return_Recordset(Return_SqlString(2, Text_Cod.Text)).Delete
        'SELECT a.*, b.tipo_telhado as tipo_telhado FROM `Equipamento` a INNER JOIN `Endereco` b on b.cod_cliente = a.cod_cliente WHERE a.cod_cliente = " & Text_Cod_Cliente.Text & " ORDER BY a.Codigo"
